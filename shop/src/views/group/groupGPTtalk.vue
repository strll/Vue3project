...
import {
  addgroup_talkGroup,
  addtodayEatGroup, deletegroup_talkGroup,
  deletetodayEatGroup, getgroup_talkGroup,
  gettodayEatGroup
} from "@/utils/request";
...
export default {
  ...
  setup(){
    ...
    const gettodayEatGrouppage=()=>{
      getgroup_talkGroup().then(response=>{
    ...
    const addroot = () => {
      addgroup_talkGroup(data.rootname).then(()=>{
        data.rootname="",
    ...
    const deleteRow = (index) => {
      ...
      ElMessageBox.confirm(
          ...
          .then(() => {
            ...
            deletegroup_talkGroup(index.row.message).then(()=>{
              data.rootname="",
                  dialogFormVisible.value = false
              data.tableData=[]
              getgroup_talkGroup().then(response=>{
