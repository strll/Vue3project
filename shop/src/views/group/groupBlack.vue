<template>
  ...
  <el-table :data="array" style="width: 100%" max-height="250" type="index">
    <el-table-column fixed prop="message" label="黑名单账号"/>
...
import {
  addblackGroup,
  addtodayEatGroup, deleteblackGroup,
  deletetodayEatGroup, getblackGroup,
  gettodayEatGroup
} from "@/utils/request";
...
export default {
  ...
  setup(){
    ...
    const addroot = () => {
      addblackGroup(data.rootname).then(()=>{
        data.rootname="",
    ...
    const deleteRow = (index) => {
      ...
      ElMessageBox.confirm(
          ...
          .then(() => {
            ...
            deleteblackGroup(index.row.message).then(()=>{
              data.rootname="",
                  dialogFormVisible.value = false
              data.tableData=[]
              getblackGroup().then(response=>{
