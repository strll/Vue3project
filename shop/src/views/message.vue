<template>
  ...
  <el-table :data="showtableData" style="width: 100%">
    ...
    <el-table-column prop="keymessage" label="触发词"  />
    <el-table-column prop="valuemessage" label="文字信息"  />
    <el-table-column prop="url" label="图片信息" >
    ...
    <el-table-column fixed="right" label="控制" width="120">
      <template #default="deletescope">
        <el-button link type="primary" size="small"  @click="deletetodayeatpage(deletescope.row.id,deletescope.row.url)">删除</el-button>
      </template>
    </el-table-column>
  </el-table>
...
export default {
  ...
  setup(){
    const likemessage=function (){
      getLikemessage(data.selectmessage).then(response=>{
    ...
    const deletetodayeatpage=function (id,url){
